...
ActionPlayer::ActionPlayer(std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, std::shared_ptr<Texture>texture) :Figure(model, shader, texture){
	
	this->Init();
}
ActionPlayer::ActionPlayer(std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, Vector4 color) : Figure(model, shader, color) {
	Init();
}
...
void ActionPlayer::CatchEvent(ImpactEvents impt, GLint next_pos = 0) {
	m_Event = impt;
