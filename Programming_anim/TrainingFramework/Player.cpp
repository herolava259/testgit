...
Figure::Figure(std::shared_ptr<Models> model, std::shared_ptr<Shaders>shader, std::shared_ptr<Texture>texture):Sprite2D(model,shader,texture){
...
Figure::Figure(std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, Vector4 color) :Sprite2D(model, shader, color) {
	m_Id = 1;
...
GLint Figure::GetnumCake() {
	return m_numCake;
}
void Figure::SetnumCake(GLint numCake) {
	m_numCake = numCake;
}

StateFigTypes Figure::GetCurrentState() {

	return m_State;
}

void Figure::AddState(const std::string& str, StateFigTypes statetype, StateAttribute stateattr = StateAttribute(0, 0.0f, 0.0f, NULL)) {
...
void Figure::SetFps(StateFigTypes state, GLint numFrame, GLfloat frameTime, GLfloat speed,bool isLoop, std::shared_ptr<Texture> textureAnimation) {
...
void Figure::SetState(StateFigTypes state) {
...
bool Figure::IsChangeState() {
	return m_isChangeState;
}
//class SpriteAnimation;
void Figure::DrawCurrState() {
...
void Figure::Updated(GLfloat deltatime) {
	if (!m_isChangeState) {
